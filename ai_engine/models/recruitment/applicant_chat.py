...
CANDIDATE: {applicant.first_name} {applicant.last_name}
...
def _call_ollama(messages: list[dict]) -> str:
    """Ollama's /api/generate is single-prompt, not chat-native, so the
    running conversation is flattened into one prompt with role labels --
    the same approach as a manual chat template."""
    flattened = "\n\n".join(
